...
BEGIN 
	inf( 'Start processing of tmp mountains extracted from OSM');
	FOR r_tmp IN ( 
		SELECT name, osm_node_id
			, ele, lat, lon
		...
		THEN
			SELECT remark INTO v_remark_curr
			FROM alph_mountain
			WHERE id = v_mount_id_curr
			;
		  	IF instr( nvl(v_remark, '?'), 'OSM_node_id' ) = 0 
			THEN 
				UPDATE alph_mountain z
				SET remark = z.remark||'. OSM_node_id '||r_tmp.osm_node_id'
					||CASE WHEN z.name_display <> r_tmp.name THEN ', name: '||r_tmp.name END 
				WHERE id = v_mount_id_curr
				;
				v_upd_cnt := v_upd_cnt + 1;
			ELSE
				inf( 'Mount id '||v_mount_id_curr||' already updated?' );
			END IF;
		ELSE	 
		...
		END IF;
	END LOOP;
	inf ( 'Mountains inserted from tmp_moutainss_json: '||v_ins_cnt
